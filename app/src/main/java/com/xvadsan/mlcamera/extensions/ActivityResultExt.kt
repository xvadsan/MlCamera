...
inline fun ComponentActivity.getContent(
    ...
    this.activityResultRegistry.register("pickImageKey", ActivityResultContracts.GetContent()) { uri ->
        contentUri.invoke(uri)
    }.launch(mimeType)
}

inline fun ComponentActivity.takePhoto(
    uri: Uri?,
    crossinline callback: (success: Boolean?) -> Unit = {}
) {
    this.activityResultRegistry.register("takePhotoKey", ActivityResultContracts.TakePicture()) { success ->
        if (success)
            callback.invoke(success)
    }.launch(uri)
}